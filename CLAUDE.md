...
7. **Sanity CDN URLs are content-hashed, not filename-derived.** Renaming an asset's `originalFilename` field updates document metadata but does not change the public URL — the URL is derived from the asset's content hash. Filename-based SEO signals operate at the metadata layer (originalFilename surfaced in ImageObject schema, alt text, surrounding content), not the rendered `<img src>` URL. Do not scope work around the assumption that renaming originalFilename will change anything in page source.
8. **Function props cannot cross the React Server Component boundary.** When rendering images inside Portable Text from a server component (e.g., `src/app/blog/[slug]/page.tsx`), do not pass function props (custom `loader`, transformers, event handlers) to `<Image>`. The prerender step will fail with "Functions cannot be passed directly to Client Components." Either construct the URL inline using `urlFor()` with width arguments, or move the rendering into a `"use client"` component that receives serializable data as props.
9. **ISR requires `export const revalidate` at page level.** Any page using `generateStaticParams()` must also export a page-level `revalidate` value if it needs to update after deploy via Sanity webhook or other revalidation triggers. Without it, Vercel treats the page as a fully static CDN asset and `revalidateTag` / `revalidatePath` calls have no effect on the rendered HTML, even though they return 200. The fetch-level `{ next: { tags, revalidate } }` option configures the data cache, not the Full Route Cache.
10. **Recon must include rendered-page parity, not just source data.** Any audit or migration involving body content must explicitly compare the rendered live page against the rendered rebuild. Source parsing alone (WXR, REST API, scraped HTML) captures what exists but not how it's grouped, laid out, or visually composed. Open the live page in a browser as part of every recon. Capture: image grouping (gallery rows vs single inline), link placement and patterns, structural elements (sidebars, callouts, asides), visual rhythm. Source-level audits that skip this step have shipped layout regressions.
11. **`priority` prop only on above-the-fold images.** The hero image on each page gets `priority`. All other images do not.
12. **SOURCE QUALITY.** Images must match or exceed live-site quality at every breakpoint. When porting from WordPress, always pull from `-scaled.jpg` or unsuffixed originals. Never use `-1200x800`, `-600x400`, or other WordPress resized variants as the source file. Verify final dimensions are at least 1600px on the longest edge for hero images and content photos. (Logos and icons exempt — native size is fine.)
13. **Content images render constrained, not full-width.** Inline illustration images inside page sections must be wrapped in `max-w-2xl mx-auto` (or a narrower cap if appropriate) with `py-8` vertical breathing room. Full-width treatment is reserved for hero images handled by the `ServiceHero` component. Never display an image wider than its native resolution — upscaling exposes source quality issues. Set the `sizes` attribute to match the actual display cap (e.g. `"(min-width: 672px) 672px, 100vw"` for `max-w-2xl`).

---

## BODY CONTENT MIGRATION RULES — WHEN REBUILDING FROM SOURCE

These rules apply any time post or page body content is reconstructed from a source (WXR export, WP REST API, scraped HTML, prior migration output). They exist because body migration carries failure modes that source parsing alone does not reveal.

1. **Use WXR `wp:attachment_url` as the authoritative source for image URLs.** WordPress filename-pattern derivation (stripping `-NNNxNNN` suffixes, `_NNN` variants, `-scaled-N` patterns) is unreliable. WP only generates resize variants when the upload is larger than the variant size. Phone-photo uploads at the variant size (e.g., `img_1200.jpg`) have no unsuffixed parent -- strip-and-fetch returns 404. The canonical original URL is in the WXR's `<wp:item>` blocks where `<wp:post_type>attachment</wp:post_type>`. Look up by attachment ID (extracted from `wp-image-NNNN` class or `_thumbnail_id` post-meta) and use `wp:attachment_url` verbatim. Do not derive URLs from filenames.

2. **`createOrReplace` wipes omitted fields.** When updating Sanity documents with a payload that omits fields (e.g., featuredImage omitted because fetch failed), `createOrReplace` will null out the omitted fields on the existing document. Migrations that touch documents must read existing state first and preserve fields they are not explicitly updating. Use `client.patch()` for field-level updates when full replacement is not intended.

3. **The full transform pipeline must be ported.** When porting body content logic from `migrate-wp-post.ts` to a new migration script, the link substitutions are NOT pure rewrites -- `applyLinkSubstitutions()` includes link insertion logic that adds internal cross-links to posts that did not have them in WP source. Audit the transform code end-to-end before authorizing a body rebuild. Confirm whether each named transform includes hidden behavior beyond its name.

4. **Idempotency on filename collisions: overwrite assets, abort on body collision.** When uploading an asset with a slug-derived filename that already exists in Sanity, overwrite -- this is the intended behavior of a re-migration. When rewriting body content on a document that already contains image blocks (i.e., a prior migration partially completed), abort with a clear warning and require an explicit `--force` flag to proceed. Silent re-migration of bodies can double-write images.

5. **Manifest emission must be incremental, not buffered.** Migration scripts must write per-post manifest entries immediately after each post completes, not buffer to end-of-run. If the script crashes on post N of 25, the manifest must already contain posts 1 through N-1 with their results. End-of-run manifest writes lose the audit trail on partial failures.

6. **Dry-run protocol is mandatory.** Before any full migration run, execute at minimum: one happy-path slug (clean fetch, no edge-case filenames) and one expected-fail slug (designed to exercise a known failure mode -- deleted asset, malformed URL, missing attachment). Dry-run output goes to `/tmp` as JSON, not Sanity. Live-run after both dry-runs pass supervisor review.

7. **Source-of-truth verification after every write.** After any Sanity write, run a GROQ query that dereferences asset references to confirm assets exist and have valid CDN URLs. A top-level GROQ query showing `bodyImageCount: 6` does not prove the references resolve -- assets can be malformed while references look clean. Dereference and verify before declaring success.

8. **Sanity document type is `blogPost`, not `post`.** Confirmed during this project. Always verify document type names from the Sanity schema before writing GROQ queries; assuming standard names silently returns empty results.
